...
export class CartNegocioService {
  ...
  async addToCart(total: number, detalle_carrito: any) {
    ...
    let carrito = this.utilsSvc.getFromLocalStorage('carrito_negocio');
    ...
    try {
      await this.firebaseSvc.updateDocumet(`users/${userId}/carrito_negocio/${carritoId}`, {
        total: carrito[0].total,
      });
      await this.firebaseSvc.setDocument(`users/${userId}/carrito_negocio/${carritoId}/detalle_carrito/${detalleCarritoId}`, detalle_carrito);
  ...
  async updateProductInCart(productId: string, cantidad: number, subtotal: number, tamano_tortilla: any) {
    const loading = await this.utilsSvc.loading();
    await loading.present();
    ...
    if (carrito[0] && Array.isArray(carrito[0].detalle_carrito)) {
      ...
      if (productIndex !== -1) {
        ...
        try {
          ...
          await this.firebaseSvc.updateDocumet(`users/${userId}/carrito_negocio/${carritoId}/detalle_carrito/${detalleCarritoId}`, {
            cantidad,
            subtotal,
          });
