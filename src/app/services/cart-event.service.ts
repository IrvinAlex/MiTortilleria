...
export class CartEventService {
  ...
  async addToCart(total: number, detalle_carrito: any) {
    ...
    let carrito = this.utilsSvc.getFromLocalStorage('carrito_eventos');
    ...
    try {
      await this.firebaseSvc.setDocument(`users/${userId}/carrito_eventos/${carritoId}/detalle_carrito/${detalleCarritoId}`, detalle_carrito);
      await this.firebaseSvc.updateDocumet(`users/${userId}/carrito_eventos/${carritoId}`, { total: carrito[0].total });
    ...
    if (carrito[0] && Array.isArray(carrito[0].detalle_carrito)) {
      ...
      if (productIndex !== -1) {
        ...
        try {
          ...
          await this.firebaseSvc.updateDocumet(`users/${userId}/carrito_eventos/${carritoId}/detalle_carrito/${detalleCarritoId}`, {
            cantidad,
            subtotal,
          });
